...
public class VehicleControl : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.transform.root.GetComponent<VehicleControl>())
        {
            ...
            collision.transform.root.GetComponent<VehicleControl>().slip2 = Mathf.Clamp(collision.relativeVelocity.magnitude, 0.0f, 10.0f); //0~10 사이의 선형 속도
    ...
    void FixedUpdate()
    {
      ...
      //  carSetting.shiftCentre.x = -Mathf.Clamp(steer * (speed / 100), -0.03f, 0.03f);
        ...
        foreach (WheelComponent w in wheels)
        {
            ...
            if(activeControl)
            { //핵심 수정
              //   Debug.Log("slip: "+slip+"\tslip2: "+slip2+ "\nstiffness= " + fc.stiffness);
              //   Debug.Log("accel: "+ accel); // accel 은 -1 ~ 0 ~ 1 0만 아니면 다똑같음
              //   Debug.Log("steer: "+ steer); //steer 또한 -1 ~ 1 
            }
            col.sidewaysFriction = fc;
